...
**Migration impact (corrected — verified live).** The REAL join table is `citas_servicios(citasId INT, serviciosId INT)` (TypeORM `synchronize` naming). Production (`VPS`, MySQL `posfinal-mysql`, DB `salon_saas`) holds exactly that table with 2 rows and was never migrated: deploys run `DB_SYNCHRONIZE=true`, so the stale `InitialSchema` names (`citaId`/`servicioId`) do NOT describe production. **We standardize on the LIVE names** (`citasId`/`serviciosId`) and add `cantidad` via an **additive** `ALTER TABLE citas_servicios ADD COLUMN cantidad INT NOT NULL DEFAULT 1` (with `down()` dropping it). No column is renamed and the table is never recreated. Local `DB_SYNCHRONIZE=true` only adds `cantidad` too, keeping the 2-row dev table intact. `CitaDTO`, `TypeORMCitaRepository`, `DisponibilidadService`, `CreateCitaUseCase` read `citasServicios.servicio`.
