...
The system MUST expose `POST /api/auth/login` accepting `{ email, password }` and returning `{ accessToken, refreshToken, user }` upon successful credential verification.
...
The system MUST support `POST /api/auth/refresh` accepting a valid refresh token and returning a new access token + new refresh token. Used refresh tokens MUST be invalidated.
