...
A tenant middleware MUST extract `salonId` from JWT claims or API key lookup and attach it to `req.salonId`. If no tenant can be determined for a protected route, the middleware MUST reject with 401.
